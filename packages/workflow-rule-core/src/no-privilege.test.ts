...
const PRIVILEGED_MUST_NOT_IMPORT_WORKFLOW_RULE_CORE = [
...
describe("workflow-rule-core cannot participate in privilege (architecture ban)", () => {
  it("workflow-rule-core production sources never mint privilege", () => {
  ...
  it("privileged decision sources never import workflow-rule-core", () => {
    for (const rel of PRIVILEGED_MUST_NOT_IMPORT_WORKFLOW_RULE_CORE) {
      const src = readFileSync(path.join(repoRoot, rel), "utf8");
      expect(src, `${rel} must not reference workflow-rule-core`).not.toContain(
        "workflow-rule-core",
      );
    }
  });

  it("privileged packages do not depend on workflow-rule-core for authority decisions", () => {
    for (const rel of PRIVILEGED_PACKAGE_JSON) {
      const pkg = JSON.parse(
        readFileSync(path.join(repoRoot, rel), "utf8"),
      ) as { dependencies?: Record<string, string> };
      expect(
        pkg.dependencies?.["@truemandate/workflow-rule-core"],
      ).toBeUndefined();
    }
  });
});
