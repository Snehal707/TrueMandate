...
describe("analytics-query cannot participate in privilege (architecture ban)", () => {
  it("privileged sources never reference analytics-query / bigquery", () => {
  ...
  it("privileged packages do not depend on analytics-query", () => {
    for (const rel of PRIVILEGED_PACKAGE_JSON) {
      ...
      expect(pkg.dependencies?.["@truemandate/analytics-query"]).toBeUndefined();
      expect(
        pkg.dependencies?.["@truemandate/analytics-bigquery"],
      ).toBeUndefined();
      expect(
